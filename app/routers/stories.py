...
def create_story(
    ...
    child = (
        db.query(Child)
        .filter(Child.id == story_data.child_id, Child.user_id == current_user.id)
        .first()
    )
    if not child:
    ...
    if not child.photo_path:
        raise HTTPException(
            status_code=400,
            detail="Child photo is required. Please upload child photo first.",
        )
    ...
    success = story_generation_service.generate_preview(
        db=db, story=story, child=child, book=book, num_pages=13
    )
